...
namespace Frugalia {
    ...
    public readonly struct Modelo {
        ...
        /// <summary>Precio de escritura manual de la caché para 5 minutos de duración mínima. La caché se mantiene activa indefinidamente si se consulta en menos de los 5 minutos de límite. El precio de escritura de los tókenes es único, es decir no se le suma precio de entrada a los tókenes que se escribirán en la caché. Claude funciona así.</summary>
        internal decimal? PrecioEscrituraCachéRefrescablePor5Minutos { get; } // Aplicable para modelos como Claude que no tienen caché automática si no manual. Si la usas constantemente (cada <5 min), la caché puede vivir “indefinidamente”.

        /// <summary>Precio de escritura manual de la caché para 60 minutos de duración mínima. La caché se mantiene activa indefinidamente si se consulta en menos de la hora de límite. El precio de escritura de los tókenes es único, es decir no se le suma precio de entrada a los tókenes que se escribirán en la caché. Claude funciona así. </summary>
        internal decimal? PrecioEscrituraCachéRefrescablePor60Minutos { get; } // Aplicable para modelos como Claude que no tienen caché automática si no manual.

        /// <summary>Costo de almacenar 1 millón de tókenes durante una hora. También se puede solicitar almacenamiento por menos de una hora y se cobra proporcional. Así funciona Gemini. No se ahorra el costo de entrada, igual se cobra. Por ejemplo, el costo de enviar a caché los 1M tókenes por 15 minutos es de 2 USD + 4,5 USD / 4 (15 minutos de almacenamiento en caché a 4,5 USD PrecioAlmacenamientoCachéPorHora.</summary>
        internal decimal? PrecioAlmacenamientoCachéPorHora { get; }

        /// <summary>Aplicable para modelos como GPT que tienen un límite a partir del cual activan la caché automática. Estos modelos no cobran el costo de escritura de caché entonces se prestan para el truco del relleno de la instrucción del sistema para lograr ahorros.</summary>
        internal int? LímiteTókenesActivaciónCachéAutomática { get; }

        public string Nombre { get; }

        /// <summary>Un modelo un nivel más grande que el actual.</summary>
        internal string NombreModelo1NivelSuperior { get; }

        /// <summary>Un modelo dos niveles más grande que el actual.</summary>
        internal string NombreModelo2NivelesSuperior { get; }

        /// <summary>Un modelo tres niveles más grande que el actual. Se guarda principalmente detectar que el modelo que tenga un modelo tres niveles superior es un modelo muy pequeño. Por el momento no se hacen saltos de modelo de tres niveles con la función modo calidad adaptable.</summary>
        internal string NombreModelo3NivelesSuperior { get; }

        public Familia Familia { get; }

        /// <summary>Máximo de tókenes de entrada que la librería permitirá para este modelo. Se usa para no exceder los límites del modelo y en casos como Claude Sonnet, evitar cruzar el umbral a modos de precio más caros (por ejemplo Sonnet en versión de 1M tókenes de entrada, a la que se le llama Sonnet+). En el caso de los otros modelos no es tan exacto porque el límite en GPT por ejemplo incluye la salida, entonces el límite real estaría un poco antes del especificado en TókenesEntradaMáximos.</summary>
        private int TókenesEntradaMáximos { get; }

        /// <summary>Límite seguro de tókenes de entrada para evitar acercarse demasiado al límite máximo del modelo. Usado internamente para evitar errores por exceder el límite.</summary>
        ...
        internal Dictionary<ModoServicio, FactoresPrecioPorModo> FactoresPrecio { get; }

        /// <summary>Funcionalidad de modelos que activan la caché automáticamente (Por ejemplo, GPT) que permiten establecer la duración de la caché por varias horas sin costo adicional. En el caso de GPT por 24 horas.</summary>
        internal bool UsaCachéExtendida { get; }
        ...
        internal Modelo(string nombre, Familia familia, decimal precioEntradaNoCaché, decimal precioEntradaCaché, decimal precioSalidaNoRazonamiento,
            decimal precioSalidaRazonamiento, decimal? precioEscrituraCachéRefrescablePor5Minutos, decimal? precioEscrituraCachéRefrescablePor60Minutos,
            decimal? precioAlmacenamientoCachéPorHora, int? límiteTókenesActivaciónCachéAutomática, int tókenesEntradaMáximos, bool usaCachéExtendida,
            Dictionary<ModoServicio, FactoresPrecioPorModo> factoresPrecio, string nombreModelo1NivelSuperior = "", string nombreModelo2NivelesSuperior = "",
            ...
            PrecioEscrituraCachéRefrescablePor5Minutos = precioEscrituraCachéRefrescablePor5Minutos;
            PrecioEscrituraCachéRefrescablePor60Minutos = precioEscrituraCachéRefrescablePor60Minutos;
            ...
            if (!ModosServicioDisponibles.TryGetValue(familia, out ModoServicio[] modosServicio))
                throw new Exception($"No hay modos de servicio disponibles para la familia {familia}.");
            foreach (var modoServicio in modosServicio) {
                if (modoServicio == ModoServicio.Normal) continue;
                if (!factoresPrecio.ContainsKey(modoServicio))
                    throw new Exception($"El modelo {nombre} de la familia {familia} no tiene definido el factor de precio para el modo de " +
        ...
        internal static double ObtenerFactorDescuentoCaché(Modelo modelo, ModoServicio modo) {

            if (!modelo.FactoresPrecio.TryGetValue(modo, out var factores))
                throw new ArgumentException($"El modo de servicio {modo} no tiene factores de precio configurados para el modelo " +
                    $"{modelo.Nombre}'.", nameof(modo));

            var precioCaché = factores.LecturaCache * (double)modelo.PrecioEntradaCaché; // Ejemplo para gpt-4.1 y Lote: FactorLecturaCaché de 2 (ajustado para reflejar que no está disponible) * PrecioEntradaCaché (Normal) de 0.5 = 1 USD/Mt.   
            var precioNoCaché = factores.EntradaYSalida * (double)modelo.PrecioEntradaNoCaché; // Ejemplo para gpt-4.1 y Lote: FactorEntradaYSalida de 0.5 * PrecioEntradaNoCaché (Normal) de 2 = 1 USD/Mt. 
            return precioCaché / precioNoCaché; // Ejemplo para gpt-4.1 y Lote: 1 / 1 = 1. Lo que es consistente con decir que no hay descuento en caché.
